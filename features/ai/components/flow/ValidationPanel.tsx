...
interface ValidationPanelProps {
  stepIndex: number;
  onValidationComplete: (isValid: boolean, feedback?: string) => void;
}
...
export default function ValidationPanel({ stepIndex, onValidationComplete }: ValidationPanelProps) {
  ...
  const handleValidate = async (isValid: boolean) => {
    ...
    try {
      ...
      if (executionTrace) {
        ...
        onValidationComplete(isValid, feedback);
      }
    } catch (error) {
  ...
  return (
    <div className="mt-4">
      {/* Status indicator for already validated steps */}
