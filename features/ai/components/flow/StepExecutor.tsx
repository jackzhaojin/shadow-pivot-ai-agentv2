...
export default function StepExecutor({ brief, setBrief }: StepExecutorProps) {
  ...
  // Simple download function that bypasses closure issues
  const handleDownload = useCallback(async () => {
    console.log('🚀 DOWNLOAD STARTED - handleDownload called');
    
    try {
      // Instead of relying on closure, get the current state directly from the context
      // This should get the most up-to-date values
      console.log('📊 Current state check:', {
        selectedFigmaSpec: !!selectedFigmaSpec,
        figmaSelectionReasoning: !!figmaSelectionReasoning,
        executionTrace: !!executionTrace,
        userGuid
      });

      // For now, let's create a simple payload with whatever we have
      const downloadPayload = {
        selectedFigmaSpec: selectedFigmaSpec || {
          name: "Generated Figma Specification",
          description: "AI-generated Figma specification",
          content: "Figma specification content"
        },
        figmaSelectionReasoning: figmaSelectionReasoning || "Generated by AI agent",
        executionTrace: executionTrace || { events: [] },
        userGuid: userGuid
      };

      console.log('� Making download request with payload:', {
        hasSelectedFigmaSpec: !!downloadPayload.selectedFigmaSpec,
        selectedFigmaSpecKeys: Object.keys(downloadPayload.selectedFigmaSpec),
        payloadSize: JSON.stringify(downloadPayload).length
      });
      ...
      if (blob.size < 500) {
        console.error('❌ Blob suspiciously small, checking content...');
      ...
      link.href = url;
      link.download = `figma-spec-${Date.now()}.zip`;
  ...
  }, [selectedFigmaSpec, figmaSelectionReasoning, executionTrace, userGuid]);

  // Download function for Figma specification
  const downloadFigmaSpec = useCallback(async () => {
    console.log('� DOWNLOAD FUNCTION CALLED - ENTRY POINT');
    console.log('�🔄 StepExecutor - downloadFigmaSpec called:', {
      hasSelectedFigmaSpec: !!selectedFigmaSpec,
      selectedFigmaSpecName: selectedFigmaSpec?.name,
    ...
    try {
      console.log('📡 StepExecutor - Making download API call to /api/agent/download-figma-spec');
      ...
      console.log('📊 StepExecutor - Request body:', {
        hasSelectedFigmaSpec: !!requestBody.selectedFigmaSpec,
        selectedFigmaSpecKeys: requestBody.selectedFigmaSpec ? Object.keys(requestBody.selectedFigmaSpec) : [],
        figmaSelectionReasoningLength: requestBody.figmaSelectionReasoning?.length || 0,
        hasExecutionTrace: !!requestBody.executionTrace,
      ...
      console.log('📨 StepExecutor - Download API response:', {
        status: response.status,
        statusText: response.statusText,
        ok: response.ok,
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error('❌ StepExecutor - Download API failed:', {
          status: response.status,
          statusText: response.statusText,
          errorText
        });
      ...
      console.log('📦 StepExecutor - Downloaded blob:', {
        size: blob.size,
        type: blob.type
      });
      ...
      console.log('💾 StepExecutor - Download details:', {
        filename,
        contentDisposition,
        blobSize: blob.size,
  ...
  useEffect(() => {
    console.log('🔍 StepExecutor - State changed, comprehensive debug:', {
      ...
      selectedFigmaSpec: {
        hasSelectedFigmaSpec: !!selectedFigmaSpec,
        selectedFigmaSpecName: selectedFigmaSpec?.name,
        selectedFigmaSpecKeys: selectedFigmaSpec ? Object.keys(selectedFigmaSpec) : []
      },
      downloadCondition: {
        currentStepGE6: currentStep >= 6,
        hasSelectedFigmaSpec: !!selectedFigmaSpec,
        shouldShowDownload: (currentStep >= 6 && selectedFigmaSpec)
      },
      aborted,
      failedStep,
      userGuid
    });
  }, [currentStep, figmaSpecs, designConcepts, evaluationResults, selectedConcept, selectedFigmaSpec, aborted, failedStep, userGuid]);
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 mb-8">
          ...
          <div className="text-center">
            ...
            <div className="text-xs text-gray-500 mb-4">
              Debug: selectedFigmaSpec = {String(!!selectedFigmaSpec)}, name = {selectedFigmaSpec?.name || 'null'}
            </div>
            ...
            <button 
              onClick={(e) => {
                ...
                // Rest of the async function will go here...
                (async () => {
